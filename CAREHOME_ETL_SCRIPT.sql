...
INSERT INTO  CAREHOME_STAGEAREA1 (SELECT ADMISSION_YEAR, CARE_CENTRE_NAME, TOWN, TOTAL_ADMISSIONS, 'WYR' FROM WYR_DATASET1);

INSERT INTO  CAREHOME_STAGEAREA1 (SELECT ADMISSION_YEAR, CARE_CENTRE_NAME, TOWN, TOTAL_ADMISSIONS, 'NYR' FROM NYR_DATASET2);
...
INSERT INTO CAREHOME_LOCATION_DIM SELECT loc_seq.nextval, CARE_CENTRE_NAME, TOWN FROM CAREHOME_STAGEAREA1;
...
DROP table CAREHOME_ADMISSION_FACT_TMP;
CREATE TABLE CAREHOME_ADMISSION_FACT_TMP AS
SELECT ADMISSION_YEAR as which_year, CARE_CENTRE_NAME, TOWN, SUM(TOTAL_ADMISSIONS) as ADMISSIONS FROM CAREHOME_STAGEAREA1
GROUP BY ADMISSION_YEAR, CARE_CENTRE_NAME, TOWN;

-- CARE HOME FACT TABLE
INSERT INTO CAREHOME_ADMISSION_FACT(ADMISSION_FACT_ID, FK1_TIME_ID, FK2_CAREHOME_ID , NO_OF_ADMISSIONS)
SELECT fact_seq.nextval,  CAREHOME_TIME_DIM.TIME_ID, CAREHOME_LOCATION_DIM.CAREHOME_ID, CAREHOME_ADMISSION_FACT_TMP.ADMISSIONS
FROM CAREHOME_TIME_DIM, CAREHOME_LOCATION_DIM, CAREHOME_ADMISSION_FACT_TMP
WHERE CAREHOME_ADMISSION_FACT_TMP.CARE_CENTRE_NAME = CAREHOME_LOCATION_DIM.CARE_CENTRE_NAME AND CAREHOME_ADMISSION_FACT_TMP.TOWN = CAREHOME_LOCATION_DIM.CAREHOME_LOCATION AND CAREHOME_ADMISSION_FACT_TMP.WHICH_YEAR = CAREHOME_TIME_DIM.YEAR;

SELECT * FROM CAREHOME_ADMISSION_FACT_TMP;